...
import { congratulationsound, tadaasound } from "../../utils/SoundManager/SoundManager";
...
export const PlatinumRankingModal = ({ steps, isVisible, onClose }) => {
    ...
    return <UpgradePetModal isVisible={isVisible} okPressed={onClose} subtitle={"Platinum  League"} bottomtext={`Total score last week:${steps} steps`} title={'Weekly Results'} backImg={rank.Platinum}
...
export const UnrankedRankingModal = ({  isVisible, onClose }) => {
    useEffect(() => {
    if (isVisible) {
      congratulationsound();
    }
